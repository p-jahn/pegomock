...
var _ = Describe("CLI", func() {
	...
	BeforeEach(func() {
		...
		packageDir = joinPath(tmpDir, "pegomocktest")
...
` + joinPath(packageDir, "mock_mydisplay_test.go")))
				...
				Context("recursive", func() {
					It("removes mock files recursively", func() {
						...
						Expect(buf.String()).To(ContainSubstring(`Deleting the following files:
` + joinPath(packageDir, "mock_mydisplay_test.go") + `
` + joinPath(packageDir, "subpackage", "mock_subdisplay.go")))
						Expect(joinPath(packageDir, "mock_mydisplay_test.go")).NotTo(BeAnExistingFile())
...
`+joinPath(packageDir, "matchers")+`
`+joinPath(packageDir, "matchers", "ptr_to_http_request.go")+`
`+joinPath(packageDir, "mock_mydisplay_test.go")+`
`+joinPath(packageDir, "mock_requesthandler_test.go")+`
`+joinPath(packageDir, "subpackage", "matchers")+`
`+joinPath(packageDir, "subpackage", "matchers", "ptr_to_http_request.go")+`
`+joinPath(packageDir, "subpackage", "mock_requesthandler.go")+`
`+joinPath(packageDir, "subpackage", "mock_subdisplay.go")+`
`), buf.String())
							Expect(joinPath(packageDir, "mock_mydisplay_test.go")).NotTo(BeAnExistingFile())
							Expect(joinPath(subPackageDir, "mock_subdisplay.go")).NotTo(BeAnExistingFile())
